...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
  Genomic foundation models encode rich sequence regularities, yet existing tools rarely answer \emph{where} in the layer stack a specific biological grammar becomes stable. We introduce \gDTR{} (\emph{Genomic Deep-Thinking Ratio}), a training-free residual-stream lens that assigns each nucleotide token a \emph{settling depth} $c(t)$: the first layer at which its representation stabilises against the post-final-norm reference. On Evo~2~7B, splice donor/acceptor sites settle $\sim 2$ layers earlier than intronic and coding contexts (Cohen's $d\!=\!-0.36$ vs.\ the pooled chr17+chr22 intron baseline), with ENCODE enhancer-like cCREs showing a smaller but measurable shift ($d\!=\!-0.12$ on the same baseline); a single chr22 calibration transfers to held-out chr17 ($94.6\%$ effect retention). Crucially, $c(t)$ is \emph{bidirectional}, not a monotone ranking of biological signal strength: editing the central GT donor motif deepens settling, whereas shuffling the flanking grammar lets the isolated motif settle $3.18$ layers earlier, so depth must be read as a two-sided signature of motif detection and context integration. Six ClinVar molecular-consequence classes also differ in the layer at which variant-induced residual disruption peaks (Kruskal--Wallis $p\!=\!3.0\!\times\!10^{-10}$, $H\!=\!53.2$, $\varepsilon^2\!=\!0.013$, a small effect that reflects the broad cross-class overlap), with synonymous substitutions peaking at the deepest layers. \gDTR{} positions settling depth as a layer-wise interpretability axis for genomic foundation models, complementing existing variant scorers.
\end{abstract}
...
Genomic foundation models, including causal LMs (Evo~2~\cite{nguyen2026evo2}, HyenaDNA~\cite{nguyen2023hyenadna}), masked LMs (the Nucleotide Transformer~\cite{dallatorre2024nt}, DNABERT-2~\cite{zhou2024dnabert2}), and bidirectional SSMs (Caduceus~\cite{schiff2024caduceus}), have catalysed a paradigm shift in functional genomics. Yet their internal layer-wise dynamics remain difficult to interpret. Existing tools ask \emph{where} models attend~\cite{abnar2020rollout,avsec2021enformer}, \emph{what} they encode~\cite{bricken2023monosemanticity,cunningham2023sparse}, or \emph{how} variants shift representations~\cite{cheng2023alphamissense,jaganathan2019spliceai}. A complementary question is largely unmeasured: \emph{where in the layer hierarchy does a biological sequence element become stable?} We use \emph{biological grammar} informally for the context-dependent integration of motif and flank.

We introduce \gDTR{}, a layer-wise readout of residual-stream commitment depth. The Deep-Thinking Ratio of~\citet{chen2026deepthinking} measures, in NLP transformers, the layer at which intermediate logit-lens distributions~\cite{nostalgebraist2020logitlens,belrose2023tunedlens,pal2023futurelens} converge within $\varepsilon$ of the final layer. \gDTR{} adapts this idea to nucleotide sequences with three changes: a cosine residual-stream lens for small genomic vocabularies; a running-minimum envelope for non-monotone genomic-model trajectories; and an Evo~2-specific post-final-norm reference that handles representational saturation before the final layer (\S\ref{sec:framework}). Fig.~\ref{fig:schematic} summarises the readout.

This paper makes one methodological contribution. We define the per-token \emph{settling depth} $c(t)$ for genomic causal LMs (\S\ref{sec:framework}), including the architectural handling required for Evo~2's pre-final saturation. The remainder of the paper validates $c(t)$ as an interpretability axis along three properties. First, $c(t)$ is \emph{transferable}: a single chr22 calibration carries to held-out chr17 with $94.6\%$ effect retention. Second, $c(t)$ is \emph{structured and bidirectionally sensitive}: splice sites and enhancer-like cCREs settle earlier than surrounding contexts, this splice signal is not explained by next-token entropy, and targeted motif/flank perturbations separate motif detection from grammar integration. Third, $c(t)$ is \emph{consequence-correlated}: ClinVar molecular-consequence classes differ in the layer at which a variant most disrupts the residual trajectory, read through the same cosine lens applied differentially. Together these properties support a unifying claim: biological grammar changes not only \emph{what} a genomic foundation model predicts, but also \emph{where} its representation settles.
...
Evo~2 tokenises at single-nucleotide (1-mer) resolution, so its next-token mass concentrates on a $\sim\!4$-symbol alphabet and the parent DTR's logit-lens JSD collapses to a near-binary, low-resolution trace; the cosine distance in the $4096$-d residual stream descends smoothly enough to threshold into $c(t)$ and avoids the unembedding projection, unreliable through Evo~2's rotations and pre-final saturation (App.~\ref{app:architectural-quirk}). We retain JSD only as a cross-lens check for variant discrimination (App.~\ref{app:auroc-detail}), where aggregation over positions restores its signal. We define the \emph{settling depth} via a running-minimum envelope,
\begin{equation}
...
\begin{figure*}[!t]
  ...
  \caption{Splice and enhancer annotations shift toward shallower settling. Both panels report Cohen's $d$ against the pooled chr17+chr22 intron baseline ($\bar c\!=\!27.72$, red dashed). \textbf{(a)} Per-context mean settling depth $\bar c$ on canonical genomic contexts (frozen $\gamma_{\cos}\!=\!0.397$). Splice donor and acceptor (blue) sit $\sim 2$ layers below the intron baseline. The asterisk on ``5$'$~UTR$^*$'' marks an entropy-coupled context (\S\ref{sec:discussion}) reported separately from the entropy-controlled hierarchy. Right of each bar: Cohen's $d$ versus the intron baseline (Mann--Whitney significance: $\,*$, $**$, $***$, $****$ at $0.05$, $0.01$, $10^{-3}$, $10^{-4}$). \textbf{(b)} Cohen's $d$ for regulatory annotations: splice donor (reference; $-0.354$), ENCODE cCRE-ELS ($-0.118$), GTEx Whole-Blood eQTL ($+0.064$), GWAS Catalog ($+0.021$). cCRE-ELS yields a measurable enhancer signal; SNP-level eQTL and GWAS shifts sit at the noise floor.}
...
\textbf{Context-level ordering.} On canonical genomic contexts (Fig.~\ref{fig:shallowness}a), splice donor and acceptor sites settle $\sim 2$ layers earlier than the intronic baseline, and the remaining contexts cluster near or above it. We read this as a descriptive layer-ordering, not as evidence that shallower settling encodes a stronger biological prior: the within-splice breakdown in App.~\ref{app:splice-canonical} shows the ordering is non-monotone in motif canonicity, consistent with the bidirectional reading of \S\ref{sec:framework}. Because the pooled-position sample is large ($\sim\!10^7$ positions), we treat Cohen's $d$ as the primary summary and Mann--Whitney $p$-values as significance checks. Among the regulatory annotations (Fig.~\ref{fig:shallowness}b), all Cohen's $d$ values are computed against the same pooled chr17+chr22 intron baseline ($\bar c\!=\!27.72$) used in panel~(a), so cross-panel and cross-annotation magnitudes are directly comparable. ENCODE cCRE-ELS enhancer-like elements~\cite{moore2020encode} are the clearest non-splice signal at Cohen's $d\!=\!-0.118$ on this baseline (vs.\ splice-donor $d\!=\!-0.354$): biologically modest but statistically robust and chr22$\to$chr17 transferable. SNP-level annotations sit at the noise floor under this baseline: GTEx eQTLs~\cite{gtex2020} (Whole-Blood chr22 significant pairs, $n\!=\!32{,}456$, $d\!=\!+0.064$) and GWAS Catalog SNPs~\cite{sollis2023gwas} ($n\!=\!6{,}900$, $d\!=\!+0.021$) shift in the opposite direction from splice/enhancer signals but with magnitudes well below the splice and cCRE-ELS effects. The 5$'$~UTR shift is reported separately because at 5$'$~UTR, $c(t)$ exhibits the strongest residual coupling with next-token entropy ($|\rho|\!\le\!0.22$ across all canonical contexts at genome-wide scale; full per-context table in App.~\ref{app:restriction-of-range}), and we therefore conservatively keep it off the entropy-controlled hierarchy used for the splice and cCRE-ELS signals.
...
Settling depth is not simply a meter of motif intrinsic strength. We exploit its two-sided behaviour (\S\ref{sec:framework}) on $1{,}000$ canonical GT-AG donors (chr22, $\pm 10$~bp pad-averaged $\bar c$). Real donors settle at $\bar c\!=\!26.77$. (i) Replacing the central GT with AA (preserving the $\pm 100$~bp flank) deepens $\bar c$ by $0.46$ layers ($d\!=\!-0.086$, paired Wilcoxon $p\!=\!2.3\!\times\!10^{-32}$): a small but reliable canonical-dinucleotide signal. (ii) Dinucleotide-shuffling the $\pm 100$~bp flank while preserving the central GT \emph{lifts} $\bar c$ to a shallower value by $3.18$ layers ($d\!=\!+0.51$, $p\!=\!4.1\!\times\!10^{-59}$): the isolated GT becomes easier to stabilise, whereas the real donor context requires deeper flanking-grammar integration. The two perturbations push $\bar c$ in opposite directions, consistent with a depth signature that reflects motif detection and context integration rather than motif strength alone. From depth alone, we cannot decisively rule out an alternative reading in which the model \emph{disengages} from uninformative noise on shuffled flanks and commits early to the lone GT (``early simplification'' rather than reduced integration). The within-splice motif breakdown in App.~\ref{app:splice-canonical} mirrors this cautionary logic.

\subsection{Variant Disruptions Peak at Consequence-Specific Layers}
\label{sec:variant-disruption}

% REVIEW FIX (page-4 + concern on ΔDcos): recast |ΔDcos| as the §2 cosine
% lens read differentially (sensitivity vs. level crossing), and state why
% the quantised c(t) cannot localise a single-nucleotide variant.
For variants we read the \emph{same} cosine trajectory differentially: $\DDcos(\ell,t)\!=\!\Dcos^{\mathrm{alt}}(\ell,t)-\Dcos^{\mathrm{ref}}(\ell,t)$. This differs from the context analysis because settling depth $c(t)$ is a quantised first-passage layer that a single substitution rarely shifts resolvably. We therefore take the layer where $|\DDcos|$ peaks, which is the depth at which the variant most disrupts the trajectory. This is the lens of \S\ref{sec:framework} read as a sensitivity rather than a level crossing: a depth signature, not a pathogenicity score. We test this on a ClinVar~\cite{landrum2020clinvar} 15-cancer-gene cohort, class-stratified by ClinVar molecular-consequence (\textsc{mc}) priority, in decreasing order: canonical-splice, nonsense, frameshift, missense, synonymous, intron.

Argmax-layer class medians read \emph{intron $<$ frameshift $<$ nonsense $<$ missense $\approx$ canonical splice $<$ synonymous} (Fig.~\ref{fig:variants}). Because these argmax layers are discrete and heavily overlapping across classes, we use Kruskal--Wallis as a rank-based omnibus test; the six classes differ globally at $p\!=\!3.0\!\times\!10^{-10}$ ($H\!=\!53.2$, $\varepsilon^2\!=\!0.013$, a small effect that quantifies the broad cross-class overlap)~\cite{kruskal1952wallis}. The only adjacent gap individually resolved by Bonferroni-corrected Dunn tests~\cite{dunn1964multiple} is nonsense $\to$ missense ($p_{\mathrm{adj}}\!<\!10^{-3}$, rank-biserial $r\!\approx\!0.09$ on the SNV cohort with $n_{\mathrm{mis}}\!=\!920$ and $n_{\mathrm{non}}\!=\!1205$; the four-thousand-variant pooled cohort gives the same magnitude to within $\pm 0.02$). The other adjacent contrasts are consistent with the global test but not individually resolved at this sample size ($n\!=\!32$ to $1{,}740$). The interpretable result is a population-level depth shift from early-truncating consequences (intron, frameshift, nonsense) toward missense and splice disruptions, and finally toward synonymous substitutions, which peak at the deepest layers. This ordering is consistent with protein-semantic information consolidating in late layers: coding-frame--dependent disruptions register earlier, and identity-preserving substitutions register only after codon-level meaning has been integrated.
...
The depth signature is \emph{bidirectional}: motif edits deepen $\bar c$ while flank-shuffles lift it, so both directions are informative about motif detection and context integration rather than a monotone biological-strength scale (\S\ref{sec:framework}, \S\ref{sec:motif-controls}). Five scope conditions then frame the interpretation. \emph{(i)} The genome-wide context-level findings (\S\ref{sec:shallowness}) are correlational readouts of representational dynamics, while the motif/flank perturbations (\S\ref{sec:motif-controls}) are interventional but limited to a single locus class; establishing causal circuits at scale will require broader interventional follow-up (e.g.\ activation patching, sparse-dictionary edits). \emph{(ii)} At genome-wide scale, no canonical context exceeds $|\rho(c, H_t)|\!=\!0.22$ (App.~\ref{app:restriction-of-range}); the entropy-decoupling argument of \S\ref{sec:shallowness} therefore holds across the full chromosome, with $5'$~UTR conservatively kept on a separate axis as it shows the largest residual coupling. \emph{(iii)} Settling depth is a \emph{diagnostic axis}, not a ranking of biological signal strength: within-class orderings can run counter to a naive canonicity prior because tighter flanking constraint, rarer-motif-driven entropy, and explicit motif detection contribute jointly (non-canonical $<$ GT-AG $<$ GC-AG donors, App.~\ref{app:splice-canonical}; promoter-element classes with mixed arm~(a)/arm~(b) signatures, App.~\ref{app:promoter-elements}). \emph{(iv)} The current entropy control isolates next-token uncertainty but not sequence composition; we therefore report promoter elements against a GC-content-matched non-promoter control in App.~\ref{app:promoter-elements}, where TATA's apparent depth shift collapses under composition matching ($d\!=\!+0.22$ vs.\ intron, $+0.03$ vs.\ GC-matched), while GC-box remains robustly deep ($d\!=\!+0.31$ vs.\ GC-matched). $k$-mer rarity and dinucleotide composition remain leading confounders elsewhere pending matched controls. \emph{(v)} Single-base splice resolution requires a per-bp readout. Adding Caduceus~\cite{schiff2024caduceus} as a second per-position model (bidirectional SSM rather than causal) would disentangle directional inductive bias from tokenisation in the cross-architecture comparison.
...
\gDTR{} establishes a training-free, layer-resolved interpretability axis for genomic foundation models: a structured, chromosome-transferable, and consequence-correlated readout of representational dynamics rather than a meter of biological signal strength. Three findings define its message. \emph{(1)} The depth signature is \emph{bidirectional}: motif edits deepen $\bar c$ while flank-shuffles lift it, so settling depth should be read as motif detection plus context integration rather than monotone signal strength. \emph{(2)} Splice sites and enhancer-like cCREs stabilise earlier than intronic/coding contexts, and the splice signal is not explained by next-token entropy. \emph{(3)} Variant-induced $\DDcos$ peaks at consequence-specific layers, with synonymous substitutions peaking deepest. Whole-genome scaling, composition-matched controls, and sparse-dictionary~\cite{cunningham2023sparse}/causal-edit~\cite{meng2022rome} integration remain for future work.
...
We discover that Evo~2~7B's last attention layer is architecturally
...
matter: (i) $\max_t |h_{30}(t)\!-\!h_{31}(t)|\!=\!0$ exactly, so layer~31
...
so layer~30 is the rotation that moves the representation into the
post-norm direction. The deepest tap not yet aligned with
the output-ready frame is therefore $L^{\star}=29$. The canonical NLP
convention of ``tap the last layer'' does not apply to Evo~2; we use
the post-final-RMSNorm state $\hnorm$ as the convergence reference and
read $L^{\star}=29$ as the empirical boundary of the pre-rotation taps.

\begin{table}[!htbp]
  \caption{Evidence that Evo~2's last attention layer is architecturally
    idle. Layer~31 is bit-identical to layer~30 in value but is a distinct
    tensor; both differ from the post-final-norm reference, identifying
  layer~29 as the deepest tap not yet aligned with the output frame.}
...
quantity living above the rotation step and the other after it, not that
$L^{\star}\!=\!29$ is a worse tap. The reference here is $\hnorm$, the
...
below the pooled intronic baseline ($\bar c\!=\!27.72$) out to $\pm 200$\,bp. On
...
$0$ ($\bar c\!=\!23.65$ and $23.64$ respectively). The asymmetry, with the donor minimum on the exonic side and the acceptor minimum on the intronic side, mirrors the asymmetric splice-grammar features (branch point and polypyrimidine tract) that lie predominantly on the intronic side of acceptors. The full
...
the per-context joint $(n, \bar c, \sigma(H_t), \mathrm{IQR}(H_t),
\rho(c, H_t))$.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lccccc@{}}
    \toprule
    Context & $n$ & $\bar c$ & $\sigma(H_t)$ & $\mathrm{IQR}(H_t)$ & $\rho(c, H_t)$ \\
    \midrule
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    Promoter element & $n$ & $\bar c$ & $d$ vs.\ intron & $d$ vs.\ GC-matched ctrl \\
    \midrule
...
\subsection{Per-Context Aggregation Sanity Check}
\label{app:agg-sanity}

Context-level summaries in the body use per-position $c(t)$ pooled over
windows: $\bar c_{\mathrm{ctx}}\!=\!|S_{\mathrm{ctx}}|^{-1}\sum_{t\in
S_{\mathrm{ctx}}} c(t)$. As a sanity check, we recomputed the
...
between the position-pooled and region-pooled per-context $\bar c$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% APPENDIX G — Reproducibility (last)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
